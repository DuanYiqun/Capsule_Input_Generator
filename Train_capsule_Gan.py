...
parser.add_argument('--sample_interval',default=500, type=str, help='interval to sample image')
...
def train(epoch):
    ...
    for i, (imgs, labels) in enumerate(trainloader):
        ...
        d_real_loss =  (adversarial_loss(confidence, valid) + capsuleloss(predict , targets)) / 2

        # random sample generators
        optimizer_G.zero_grad()
        _, max_length_indices = predict.max(dim=1)
        y = Variable(torch.eye(NUM_CLASSES)).index_select( dim=0, index=max_length_indices.cpu().data).to(device)
        adjust = y*predict
        noisevector = torch.randn(adjust.size(0), 10, 16).to(device)
        norm2value = torch.norm(noisevector,p=2, dim = -1).to(device)
        scalar = adjust/norm2value
        nz = scalar[:,:,None]*noisevector
        ...
        gen_imgs = Gnet(nz)
        predict_fake, logits_fake, capsule_fake,confidence_fake = Dnet(gen_imgs)
        g_loss = 0.5 * (adversarial_loss(confidence_fake, valid) + capsuleloss(predict_fake, predict))
        g_loss.backward(retain_graph=True)
        optimizer_G.step()
        
        optimizer_D.zero_grad()
        ## train discriminator
        d_fake_loss = 0.5 * (adversarial_loss(confidence_fake, fake) + 0.9 * capsuleloss(predict_fake , predict))
