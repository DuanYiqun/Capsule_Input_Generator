...
parser.add_argument('--sample_interval',default=100, type=str, help='interval to sample image')
parser.add_argument('--clip_value',default=0.01, type=str, help='clip')
parser.add_argument('--n_critic',default=5, type=str, help='d times')
...
optimizer_G = torch.optim.RMSprop(Gnet.parameters(), lr=args.lr)
optimizer_D = torch.optim.RMSprop(Dnet.parameters(), lr=args.lr)
...
def sample_image(n_row, batches_done):
    ...
    # Sample noise
    z = Variable(FloatTensor(np.random.normal(0, 1, (n_row**2, 100)))).to(device)
    # Get labels ranging from 0 to n_classes for n rows
    labels = np.array([num for _ in range(n_row) for num in range(n_row)])
    labels = Variable(LongTensor(labels)).to(device)

    gen_imgs = Gnet(z, labels)
...
def train(epoch):
    ...
    for i, (imgs, labels) in enumerate(trainloader):
        ...
        if i % args.n_critic == 0:
            ...
            g_loss = 0.5 * (-torch.mean(confidence) + \
                        0.5*capsuleloss(predict, gen_targets))
        ...
        #d_real_loss =  (adversarial_loss(confidence, valid) + 0.5*capsuleloss(predict , targets)) / 2
        d_real_loss =  (-torch.mean(confidence) + 0.5*capsuleloss(predict , targets)) / 2
        # Loss for fake images
        predict_fake, logits_fake, capsule_fake,confidence_fake = Dnet(gen_imgs.detach())
        #Dnet(gen_imgs.detach())
        #d_fake_loss =  (adversarial_loss(confidence_fake, fake) + 0.5*capsuleloss(predict_fake, gen_targets)) / 2
        d_fake_loss =  (torch.mean(confidence_fake) + 0.5*capsuleloss(predict_fake, gen_targets)) / 2 
