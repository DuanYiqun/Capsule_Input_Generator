...
if  __name__ == '__main__':
    nz = torch.randn(2,110)
    Gnet = _netG(ngpu=0, nz=110)
    output = Gnet(nz)
    print(output.size())
    image_large = torch.randn(2,3,128,128)
    Dnet = _netD(ngpu=0, num_classes=10)
    significance, classes = Dnet(image_large)
    print(classes.size())

    cifarGnet = _netG_CIFAR10(ngpu=0, nz=110)
    output = cifarGnet(nz)
    print(output.size())

    cifarDnet = _netD_CIFAR10(ngpu=0)
    significance, classes = cifarDnet(output)
    print(classes.size())
